### Instructions! ###
# Because we haven't learned onStep() yet you have to use onKeyHold as a clock
# instead.
# To play hold H and use W/Space/Up to jump over the cacti. This is more or
# less a recreation of <chrome://dino>.
### Instructions! ###
...
app.str = Label(
    'Hold H to Play', 200, 200,
    fill=app.colors['greyple'],
    font='monospace', size=8*4, bold=True
)
########## END SETTINGS ##########
...
# def onStep():
    if not ('h' in k): app.stop()
    app.str.visible = False
    ...
    for i in activeCacti:
        if (app.dino.hitsShape(i)):
            app.str.visible = True
            app.str.value = 'Game Over!'
            app.stop()
